# Historical reference — previous collector and FLUT workflow

This record preserves prior implementation details. Current everyday instructions are in [the industry guide](industry_workflow.md). Paths and links below were originally relative to gaming/.

# researchOS: official state gaming revenue

**September 12 refresh:** paths in this guide are relative to `gaming/`. The latest capture has 19,806 observations; start with notebooks 94 → 95 → 96 and the [current analyst review](flut_current_quarter_review_20260912.md). The [workflow guide](flut_workflow.md) explains refresh, backup, and dated expectations. The exact earlier approved snapshots remain missing; see [data recovery](data_recovery.md). Use the root [launch and validation commands](../../README.md).
...
Open notebook 94 for current capture coverage. The older [coverage report](nationwide_coverage.md) describes every state/product,
...
Read every collection exception before using a new capture. See [refresh instructions](flut_workflow.md).
...
parser. See [Kentucky recovery](kentucky_recovery_20260912.md).
