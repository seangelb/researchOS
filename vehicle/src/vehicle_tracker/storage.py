...
def retain_capture(capture: dict, raw_directory: Path) -> Path:
    ...
    path = raw_directory / f'{digest}.json'
    if path.exists():
...
def store_capture(database: Path, *, run_id: str, page_number: int, raw_file: Path,
    ...
    """Append one page attempt atomically; same run/page cannot silently overwrite.
    ...
    try:
        ...
        with connection:
            ...
            connection.execute('''INSERT INTO vehicle_captures VALUES (?,?,?,?,?,?,?,?,?,?,?,?)''',
                (run_id, page_number, capture.get('captured_at_utc'), capture.get('page_url'),
            ...
            if not frame.empty:
                ...
                connection.executemany('INSERT INTO vehicle_observations VALUES (' + ','.join(['?'] * 16) + ')',
                    [(run_id, page_number, *row) for row in values.itertuples(index=False, name=None)])
        return len(frame)
    finally:
        connection.close()
